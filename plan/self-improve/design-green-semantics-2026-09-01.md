...
## 6c. 編輯角色的回測（2026-09-01，實測結果：可用，但只能當簡報不能當閘門）

owner 提議「每次修完派 Opus agent 判斷是否收斂」。**不是用猜的決定，是拿 run1 的三輪真實
findings 做了一次回測**：一個空 context 的 Opus agent，只看審查軌跡（不看規格本體），
被要求扮演編輯——逐條裁決、整體 SHIP/BLOCK、不准模稜兩可、不准建議「再加一層」除非說明
新風險、必須聲明利益衝突。

### 結果

**沒有變成第四個審查者**（五條全是審查者原話，它只做分級與根因合併，明文聲明未新增技術問題）、
**沒有蓋橡皮圖章**（判 BLOCK，並把 5 條裡的 2 條降級為 NON-BLOCKING）、
**沒有模稜兩可**（有界退出條件：三個改動，其中兩個是刪東西）。

它推翻了先前的框架，而且是對的：

> **審查者本身沒有問題。** R2 宣告 R1 那幾條收口後不再回頭，R3 也是。它對看過的東西是嚴格收斂的。
> 撐住 findings 數的**只有新料**。⇒ **發散的不是審查迴圈，是寫規格的迴圈。**

它做的「被打的東西幾歲」結算：R1 打原始工件 7/7；R2 打新機制 4/4；R3 打新機制 5/5。
行數 242→279(+37)→318(+39)，兩個速率都沒下降 ⇒ **不是收斂，是穩態**。

並指出 **C9 是自發增生物**——它沒有對應任何 R1/R2 的 finding，是規劃者自己加的，
而 R3 有 40% 的 findings 打的是它。且 C9 監管的是**審查流程本身**，誕生在審查流程
變成主題的那一輪——迴圈離題最清楚的自指徵兆。

### 它給了可否證條件，我們跑了

它列出三條「幾分鐘可重現」的測試，任一不重現該條即作廢。結算：

| 它判 BLOCKING 的 | 實測 | 結論 |
|---|---|---|
| B3（force restart 後痕跡被洗掉） | `--start` 已對 `restarted_over` 非空拒絕覆蓋，rc=2 | **作廢** |
| B2（`--revision` 未綁 log 內容） | 該機制在縮小範圍時已刪除，程式碼裡不存在 | **失效** |
| B1（重新 freeze 洗掉 SPEC_DRIFT） | **在 0.2.0 上重現** | **成立，已修** |

⇒ **它的 BLOCK 收斂到一條，而那一條是真的。** 修法用已存的 `sha` 做內容綁定（無新機制）。

### 對這個機制的結論

- **可用，但只能是給編輯（owner）的簡報，不能是閘門。** 讓它的裁決直接等於 `green_allowed`
  就回到 Greptile 那個坑（§3）。閘門留在腳本。
- **不要每輪都派**——每輪多一個產出就是多一片新表面，正是本文件記錄的病。先跑免費的機械
  delta，觸發了才派。
- **要問編輯的問題，不要問「收斂了嗎」**。實測有效的問法是「剩下這些意見，哪幾條夠格擋關？」
- **要求可否證條件**。它列的三條重現測試，成本遠低於一輪 Codex 複審，而且直接把
  三分之二的 BLOCKING 作廢掉了。這是這次回測最有價值的產物。

### 誠實記錄：這次回測不乾淨

派工單裡的佔位字串未被代換（監督者疏漏），該 agent 自行到 repo 挖原始檔，過程中看到了
R3 之後的產物。它主動聲明，但無法驗證。

**可信度的支撐**：它的結論與實際走向**不一致**（我們縮小範圍後出貨，它判 BLOCK），
而在「拆規格、刪 C9」上一致。純粹複述事後結果不會產生這種分歧。
日後若要再測，派工單必須明令「不得讀取任何檔案，只依本訊息作答」。
