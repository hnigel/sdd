...
### R2（2026-09-05）：六條，findings 8 → 6，**沒有觸發 `divergence_hint`**

`medium` effort、`rc=0`、`--prompt-block` 自動帶複審規則（0.6.0 的功能第一次真的用到）。

⚠️ **六條裡沒有一條是草案原本的問題 —— 全部是修 R1 時動出來的。**

| 類型 | 幾條 | 內容 |
|---|---|---|
| **加了新段落卻沒改掉舊段落** | 4 | §6.1 還寫「最近 delta」、§7 第 4 步還是無條件拒絕、結尾還寫「下一步 W1–W6」、W1–W6 與驗收矩陣仍是砍前版本 |
| 指向修正本身的新缺陷 | 2 | 鎖修法的 TOCTOU；承重守衛只認關鍵字 |

⇒ **那四條正是 M7 的病，而 M7 是兩天前才寫進 HANDOFF 第 9 條的。**
「刪一節／改一條規則之後，指向舊版的文字不會自己消失」——
這次不是指標失效，是**兩條互相矛盾的規則同時存在**，更難發現。
機械守衛（第 3b 項）只抓得到指向不存在章節的指標，抓不到這種。**缺口記著。**

處置：前三條已改；第四條（W1–W6 與驗收矩陣）**沒改，誠實標明**實作前必須先縮 ——
改它要重寫兩大段，那會製造新的未審表面。
兩條程式缺陷：承重守衛已加 body 行數下限；鎖的 TOCTOU **不宣稱關閉**（見 §6）。

### owner 裁定：停在 R2，不送 R3（2026-09-05）

`--status S3` 是 `rounds_done: 2`、`green_allowed: true`、帶 `green_is_weak_signal`。

⚠️ **這不是通過。** 停在這裡是**裁定**，不是腳本的停止條件（那是 R3）。
規格**沒有凍結**，這份草案**不是 GREEN**。

判準來自 §2：撐住 findings 數的只有每輪修訂新增的材料。
R2 的六條百分之百是修 R1 時動出來的 ⇒ R3 會審 R2 動出來的東西，
產出會是「關於上一輪修訂」而不是「關於這份設計」。

⇒ **下一步是設計文件自己寫的 P0**：先用現有版本在下游跑 3–5 件真任務（M6 的三個數字）。
那會告訴我們這 14 條 finding 裡哪幾條在真實使用中真的會咬人 —— **R3 給不了那個資訊**。

⚠️ 這一輪也順帶驗了兩件本來沒資料的事：
`--prompt-block` 自動帶複審規則**真的會用到**；
`green_is_weak_signal`（0.7.0 的 C1-6）**在真實情境下第一次作用** ——
八條全 `--resolve` 完之後它就跳出來，擋住了「resolve 完就當通過」那條路。
